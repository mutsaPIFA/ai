...
COLORS = ["블랙", "화이트", "네이비", "그레이", "베이지", "브라운", "카멜", "그린", "핑크", "블루", "스카이블루", "레드", "옐로우", "카키", "퍼플", "오렌지", "기타"]
...
color:
블랙 | 화이트 | 네이비 | 그레이 | 베이지 | 브라운 | 카멜 | 그린 | 핑크 | 블루 | 스카이블루 | 레드 | 옐로우 | 카키 | 퍼플 | 오렌지 | 기타
...
- 청색 계열 구분: 밝고 연한 하늘색 → "스카이블루", 선명한 파랑 → "블루", 어두운 남색 → "네이비"
...
class AiProductIn(BaseModel):
    ...
    color: str
    material: str
    # 카탈로그 배치 태깅 산출물 (backend #11) — 시드 태깅 전이거나 구버전 backend면 없을 수 있음
    mood: str | None = None
...
RECOMMEND_PROMPT = (
    ...
    "1. 단순 태그 일치가 아니라 색상·소재·무드, 그리고 제품의 styleNote(스타일 한줄 요약)까지 종합해 조화를 판단하세요.\n"
...
OUTFITS_PROMPT = (
    ...
    "5. 색상, 소재, 무드, 제품 styleNote(스타일 한줄 요약), 격식도와 아이템 간 조화를 종합적으로 고려하세요.\n"
    ...
    "단, 어떤 두 코디도 같은 아이템·제품을 2개 이상 공유하면 안 됩니다. 재료가 정말 부족할 때만 1~2개로 줄이세요.\n"
